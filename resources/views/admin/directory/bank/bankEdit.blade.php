...
@section('content')
    ...
    @endphp
    <form class="status formCustomSubmit" action="{{route('bankEditAjax')}}">
        @csrf
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-10">
                <input type="text" class="form-control" name="uuid" value="{{$bank->uuid}}" id="uuid" required>
            </div>
        </div>
        <div class="form-group row">
            <label for="bic" class="col-sm-2 col-form-label">Bic</label>
            <div class="col-sm-10">
                <input type="text" class="form-control" name="bic" id="bic" value="{{$bank->bic}}" required>
            </div>
        </div>
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-10">
                <textarea type="text" class="form-control" name="description" id="description">{{$bank->description}}</textarea>
            </div>
        </div>
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-10">
                @foreach($bank->parentFields as $kfield=>$parentField)
                ...
                <div class="row addItemSelect">
                    <div class="col-sm-9">
                        <x-adminlte-select2 id="parentFields{{count($bank->parentFields)+1}}" name="parentFields[{{count($bank->parentFields)+1}}][]" multiple>
                            @foreach($fields as $fieldOne)
                                @foreach($fieldOne['value'] as $fieldVal)
                                    @if(!empty($fieldVal['uuid']))
                                        <option value="{{$fieldVal['uuid']}}">{{$fieldOne['name']}}: {{$fieldVal['name']}} [{{$fieldVal['uuid']}}]</option>
        ...
        <div class="form-group row">
            <div class="col-sm-10">
                ...
                <a href="{{route('bankList')}}" style="margin-left: 10px" class="btn btn-secondary btn-md active" role="button" aria-pressed="true">Вернуться</a>
            </div>
        </div>
    </form>
