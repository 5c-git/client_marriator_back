...
//            'scope_of_services' => $taskRequest->scope_of_services??$task->scope_of_services
    ...
    public function createBidFromOrder(User $user, int $orderId, int $orderActivityId): Bid
    {
        ...
        $bids = $order->bids?->where('activity_id', $orderActivityId);
        if ($bids) {
            foreach ($bids as $bid){
    ...
    public function getSpecialistForBid(int $bidId): Collection
    {
        ...
        if($fieldView && $bid->viewActivity && $place) {
            ...
            $users = User::whereIn('data->'.$fieldStat->uuid, $status)
                ->where(function ($query) use ($viewActivitiesUuids, $fieldView) {
                    foreach ($viewActivitiesUuids as $uuid) {
                        $query->orWhereJsonContains('data->'.$fieldView->uuid, $uuid);
                    }
                })
