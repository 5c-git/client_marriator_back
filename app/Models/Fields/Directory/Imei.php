...
class Imei extends Model implements ModelDirectoryInterface
{
    use HasFactory;

    public static int $fieldsTypeEnum = FieldsTypeEnum::text->value;
    public static string $uuid = 'directory_imei';

    protected $table = 'directory_imei';
    protected $fillable = [
        'uuid',
        'name',
        'active'
    ...
    {
        $imei = self::query()->where('active', true)->inRandomOrder()
            ->first();
        if (!empty($imei)) {
            return $imei->name;
        }
