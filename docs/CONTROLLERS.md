...
Контроллеры модулей расположены в `app/Modules/` и зарегистрированы через собственные провайдеры.

### 15.1 QuestionnaireController (`app/Modules/Questionnaire/Http/Controllers/`)

| Метод | Route | Описание |
|---|---|---|
| start | POST `/api/questionnaire/start/{user}` | Создаёт/обновляет анкету и запускает цепочку шагов |
| status | GET `/api/questionnaire/status/{user}` | Возвращает статус и текущий шаг |
| result | GET `/api/questionnaire/result/{user}` | Возвращает полные данные и логи |

### 15.2 Yandex.Smena Module (`app/Modules/YandexSmena/`)

| Метод | Route | Описание |
|---|---|---|
| `POST` | `/api/yandex-smena/publish-shift` | `PublishYandexSmenaShiftController` — публикует `OrderActivities` или `TaskActivity` в Яндекс.Смена. Принимает пару `orderId + orderActivityId` или `taskId + taskActivityId`. |

Incoming events are retrieved by polling the Yandex.Smena API using:

```bash
php artisan yandex-smena:poll-events
```

This command is scheduled every minute in `routes/console.php`.

Подробнее см. `docs/YANDEX_SMENA_MODULE.md`.
